...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${image}></img>
      ...
      <h4>${product.title}</h4>
      <span class="text-danger fs-5 fw-bolder me-3">${product.rating.rate} *</span> 
      <span class="text-primary fs-5">${product.rating.count} ratings</span>
      <p class="text-danger">Category: ${product.category}</p>
      <h3>Price: $ ${product.price}</h3>
      <button onclick="addToCart(${product.price})" id="addToCart-btn" class="buy-now btn btn-outline-success fs-5">add to cart</button>

      <button onclick="displayProductDetails(${product.id})" class="btn btn-outline-danger fs-5" type="button" data-bs-toggle="modal" data-bs-target="#staticBackdrop">Details</button></div>
      `;
...
// get string input & convert it into float number 
...
//  dissplay product details in a modal 
...
const productDetails = (product) =>{
  ...
  const displayDetails = document.getElementById('product-details');
  const div = document.createElement('div');
  div.innerHTML = `
